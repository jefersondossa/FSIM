...
void VTUGenerator::PrintResultsIsoparametric(CompMesh *cmesh, std::string filename, std::vector<std::string> &scalnames, std::vector<std::string> &vecnames, const std::vector<CustomCellField>& custom_cel_fields, int step){
    ...
    for (int iel = 0; iel < cmesh->NElements(); iel++){
        auto compel = cmesh->ElementVec()[iel];
        if (compel->Dimension() != cmesh->Dimension()) continue;
        WeakForm* weak = compel->GetWeakForm();
        if (weak == nullptr || 
            weak->Type() == WeakFormType::kMixedGlobalLocalEnrichment || 
            weak->Type() == WeakFormType::kGlobalLocalEnrichment ||
            weak->Type() == WeakFormType::kInterpolatedBC ||
            weak->Type() == WeakFormType::kL2Projection
        ) continue;

        elsToPrint.insert(compel->Index());
    }

    auto graphmesh = cmesh->GetGraphMesh(elsToPrint);
    ...
    for (int i=0; i<graphmesh->NElements(); i++){
        auto graphconnect = graphmesh->Connect(i);
        for (int k = 0; k < graphconnect.size(); k++)
        {
            output_v << graphconnect[k] << " ";
        }
        output_v << std::endl;
    };
    output_v << "      </DataArray>" << std::endl;
    ...
    int aux = 0;
    for (int i=0; i<graphmesh->NElements(); i++){
        aux += cmesh->ElementVec()[graphmesh->GraphElementToMeshElement(i)]->Reference()->NGeometricNodes();
        output_v << aux << std::endl;
    };
    output_v << "      </DataArray>" << std::endl;
    ...
    for (int i=0; i<graphmesh->NElements(); i++){
        output_v << cmesh->ElementVec()[graphmesh->GraphElementToMeshElement(i)]->Reference()->PrintType() << std::endl;
    };
