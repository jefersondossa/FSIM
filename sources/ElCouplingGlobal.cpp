    ...
    if (Mesh()->getProblemParameters().ProbType() == ProblemType::EPoisson){
        for (int i = 0; i < Mesh()->NElNodes(); i++){
            for (int j = 0; j < Mesh()->NElNodes(); j++){
                double l2 = Mesh()->getNumericalIntegration()-> phi_(i,index) * phiGlobal[j] * WJ * k1;
...
void ElCouplingGlobal::ApplyBC(std::vector<MatrixDouble> &Stiffness, std::vector<VecDouble> &Rhs){
    return;
    int DIM = Mesh()->Dimension();
    ...
    if (Mesh()->getProblemParameters().ProbType() == ProblemType::EPoisson){
        for (int i = 0; i < Mesh()->NElNodes(); i++){
            ...
            if (fMeshVector[0]->NodeVec()[connectlocal[i]] -> getConstrains(0) == 1) {
                ...
                // }
                Rhs[0][i] = 0.0;
                // Rhs[0][Mesh()->NLocDOF() + i] = 0.0;
                Rhs[1][i] = 0.0;
                // Rhs[1][Mesh()->NLocDOF() + i] = 0.0;
            };
        }
    } else {
        for (int i = 0; i < Mesh()->NElNodes(); i++){
...
void ElCouplingGlobal::ArlequinStabStiffness(int &index, MatrixDouble &dphi_dx, VecDouble &phiGlobal, MatrixDouble &dphi_dxGlobal, double &weight_, double &djac_, std::vector<MatrixDouble> &Stiffness){
    ...
    if (Mesh()->getProblemParameters().ProbType() == EPoisson){
        ...
        MatrixDouble matDGlob(3,2*Mesh()->NElNodes());
        matD.setZero();
        matDGlob.setZero();
        ...
        //         AM = Mesh()->getNumericalIntegration()->phi_(i,index) * Mesh()->getNumericalIntegration()->phi_(j,index) * tARLQ_ * wna_* alpha_m;
        ...
        //         double Amx = - Mesh()->getNumericalIntegration()->phi_(i,index) * am_[k] * tARLQ_;
...
void ElCouplingGlobal::ArlequinStabResidual(int &index, MatrixDouble &dphi_dx, VecDouble &phiGlobal, MatrixDouble &dphi_dxGlobal, double &weight_, double &djac_, std::vector<VecDouble> &Rhs){
    ...
    if (Mesh()->getProblemParameters().ProbType() == EPoisson){
        ...
        for (int i = Mesh()->NElNodes(); i--; ){
            Rhs[1][2*Mesh()->NElNodes()+2*i  ] += aux[2*i];
            Rhs[1][2*Mesh()->NElNodes()+2*i+1] += aux[2*i+1];
            double shapeFi = phiGlobal[i];
            //External force
            double Fx = (fieldForce[0] + forcingF[0]) * shapeFi;
            double Fy = (fieldForce[1] + forcingF[1]) * shapeFi;
            Rhs[1][2*Mesh()->NElNodes()+2*i  ] += Fx * WJ;
            Rhs[1][2*Mesh()->NElNodes()+2*i+1] += Fy * WJ;
        };
    } else {
        return;
        PanicButton();
