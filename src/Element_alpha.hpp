...
private:
    ...
    double        ax_, ay_;      //Interpolated acceleration
    ...
    double        dumesh_dx, dumesh_dy,//Interpolated mesh 
                  dvmesh_dx, dvmesh_dy;//velocity derivatives
...
public:
    ...
    void setTimeIntegrationScheme(double& b){
        timeScheme_ = b;
        alpha_f = 1. / (1. + timeScheme_);
        alpha_m = 0.5 * (3. - timeScheme_) / (1. + timeScheme_);
    ...
    void getElemMatrixInitial(int index);
    ...
    void getResidualVectorInitial(int index);
...
//------------------------------------------------------------------------------
template<>
void Element<2>::getParameterArlequin() {
...
void Element<2>::getElemMatrix(int index){
    ...
    for (int i = 0; i < 6; i++){
        for (int j = 0; j < 6; j++){
            ...
            double wSUPGi = (una_ - umesh_) * dphi_dx(0,i) + (vna_ - vmesh_) * dphi_dx(1,i);
            double wSUPGj = (una_ - umesh_) * dphi_dx(0,j) + (vna_ - vmesh_) * dphi_dx(1,j);
            
            //Mass matrix (use for both directions)
            double mM = phi_(i) * phi_(j) * dens_ * alpha_m + 
                        wSUPGi * phi_(j) * tSUPG_ * dens_ * alpha_m;

            double sMxx = dphi_dx(0,i) * phi_(j) * ax_ * tSUPG_ * dens_ * alpha_m*0;
            double sMxy = dphi_dx(1,i) * phi_(j) * ax_ * tSUPG_ * dens_ * alpha_m*0;
            double sMyx = dphi_dx(0,i) * phi_(j) * ay_ * tSUPG_ * dens_ * alpha_m*0;
            double sMyy = dphi_dx(1,i) * phi_(j) * ay_ * tSUPG_ * dens_ * alpha_m*0;
            ...
            //Convection matrix
            double Cxx = (dphi_dx(0,j) * (una_ - umesh_) + dphi_dx(1,j) * (vna_ - vmesh_)) * phi_(i) 
            ...
            double Quu = dphi_dx(0,i) * dp_dx * phi_(j) * tSUPG_ * 0.;
            double Quv = dphi_dx(1,i) * dp_dx * phi_(j) * tSUPG_ * 0.;
            double Qvu = dphi_dx(0,i) * dp_dy * phi_(j) * tSUPG_ * 0.;
            double Qvv = dphi_dx(1,i) * dp_dy * phi_(j) * tSUPG_ * 0.;
            ...
            jacobianNRMatrix(2*i  ,2*j  ) += (mM + sMxx + Kxx + KLSxx + Cxx + Cuu + Quu) * weight_ * djac_ * intPointWeightFunction(index);
            jacobianNRMatrix(2*i+1,2*j+1) += (mM + sMyy + Kyy + KLSyy + Cyy + Cvv + Qvv) * weight_ * djac_ * intPointWeightFunction(index);
            jacobianNRMatrix(2*i  ,2*j+1) += (sMxy + Kxy + Cuv + KLSxy + Quv) * weight_ * djac_ * intPointWeightFunction(index);
            jacobianNRMatrix(2*i+1,2*j  ) += (sMyx + Kyx + Cvu + KLSyx + Qvu) * weight_ * djac_ * intPointWeightFunction(index); 
            ...
            double QSUPGx = - (dphi_dx(0,i) * phi_(j)) + 
                             ((dphi_dx(0,i) * (una_ - umesh_) + 
                               dphi_dx(1,i) * (vna_ - vmesh_)) * dphi_dx(0,j) * tSUPG_);
            //multiply pressure direction y
            double QSUPGy = - (dphi_dx(1,i) * phi_(j)) +
                             ((dphi_dx(0,i) * (una_ - umesh_) + 
                               dphi_dx(1,i) * (vna_ - vmesh_)) * dphi_dx(1,j) * tSUPG_);
            ...
            jacobianNRMatrix(12+j,2*i  ) += Qx * weight_ * djac_ * intPointWeightFunction(index);
            jacobianNRMatrix(12+j,2*i+1) += Qy * weight_ * djac_ * intPointWeightFunction(index);
...
//------------------------------------------------------------------------------
template<>
void Element<2>::getElemMatrixInitial(int index){
    ...
    for (int i = 0; i < 6; i++){
        ...
        if ((model == false) && (nodes_[connect_(i)] -> getDistFunction() > -1.2))  {
           ...
           // std::cout << "ELEMN " << index_ << " " << connect_(i) << std::endl;
            for (int j = 0; j < 18; j++){
                jacobianNRMatrix(2*i  ,j) *= 1000.e3;
                jacobianNRMatrix(j,2*i  ) *= 1000.e3;
                jacobianNRMatrix(2*i+1,j) *= 1000.e3;
                jacobianNRMatrix(j,2*i+1) *= 1000.e3;
            };
            rhsVector(2*i  ) *= 1000.e3;
            rhsVector(2*i+1) *= 1000.e3;
        };
...
void Element<2>::getResidualVector(int index){
    ...
    for (int i = 0; i < 6; i++){
        ...
        double mx = phi_(i) * ax_ * dens_ + 
                    ((una_ - umesh_) * dphi_dx(0,i) + (vna_ - vmesh_) * dphi_dx(1,i)) * 
                     (unm_ - uPrev_) * tSUPG_ * dens_;
        double my = phi_(i) * ay_ * dens_ + 
                    ((una_ - umesh_) * dphi_dx(0,i) + (vna_ - vmesh_) * dphi_dx(1,i)) * 
                     (vnm_ - vPrev_) * tSUPG_ * dens_;
        ...
        double Cx = (duna_dx * (una_ - umesh_) + duna_dy * (vna_ - vmesh_)) * phi_(i) * dens_ +
                    ((una_ - umesh_) * dphi_dx(0,i) + (vna_ - vmesh_) * dphi_dx(1,i)) *
                    ((una_ - umesh_) * duna_dx + (vna_ - vmesh_) * duna_dy) * tSUPG_ * dens_;
        double Cy = (dvna_dx * (una_ - umesh_) + dvna_dy * (vna_ - vmesh_)) * phi_(i) * dens_ +
                    ((una_ - umesh_) * dphi_dx(0,i) + (vna_ - vmesh_) * dphi_dx(1,i)) *
                    ((una_ - umesh_) * dvna_dx + (vna_ - vmesh_) * dvna_dy) * tSUPG_ * dens_;
 
        double Px = - (dphi_dx(0,i) * p_)
                    + ((dphi_dx(0,i) * (una_ - umesh_) + dphi_dx(1,i) * (vna_ - vmesh_)) * dp_dx * tSUPG_);
        double Py = - (dphi_dx(1,i) * p_) 
                    + ((dphi_dx(0,i) * (una_ - umesh_) + dphi_dx(1,i) * (vna_ - vmesh_)) * dp_dy * tSUPG_);
        ...
        double Fx = fieldForce_(0) * phi_(i);
        double Fy = fieldForce_(1) * phi_(i);

        rhsVector(2*i  ) += (-mx -Kx - Px - Cx - KLSx + Fx) * weight_ * djac_ * intPointWeightFunction(index);
        rhsVector(2*i+1) += (-my -Ky - Py - Cy - KLSy + Fx) * weight_ * djac_ * intPointWeightFunction(index);
        rhsVector(12+i) += -Q * weight_ * djac_ * intPointWeightFunction(index);
...
//------------------------------------------------------------------------------
template<>
void Element<2>::getResidualVectorInitial(int index){
...
void Element<2>::getTransientNavierStokesInitial(){
    ...
    for(typename NormalQuad::QuadratureListIt it = nQuad.begin(); 
        ...
        //Computes the element diffusion/viscosity matrix
        getElemMatrixInitial(index);

        //Computes the RHS vector
        getResidualVectorInitial(index); 
