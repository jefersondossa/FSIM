...
void Element<2>::setBoundaryConditions(){
    ...
    // for (int i = 0; i < 6; i++){
    //     x = nodes_[connect_(i)] -> getCoordinates();
    //     if((x(0) < 0.001) && ((x(1) > 3.99) || (x(1) < 0.01))){            
    //         for (int j = 0; j < 18; j++){
    //             jacobianNRMatrix(2*i+1,j) = 0.;
    //             jacobianNRMatrix(j,2*i+1) = 0.;
    //         };
    ...
    for(typename NormalQuad::QuadratureListIt it = nQuad.begin(); 
        ...
        for (int i = 0; i < 6; i++){
            ...
            if (iTimeStep < 10){
                ...
                // LLx = -(dphi_dx(0,i) * (dLx_dx/intPointWeightFunction(index) - du_dx*du_dx - dv_dx*du_dy - u_*du_dxx - v_*du_dxy) + 
                //         dphi_dx(1,i) * (dLx_dy/intPointWeightFunction(index) - du_dy*du_dx - dv_dy*du_dy - u_*du_dxy - v_*du_dyy)) * tARLQ_ / dens_;
                // LLy = -(dphi_dx(0,i) * (dLy_dx/intPointWeightFunction(index) - du_dx*dv_dx - dv_dx*dv_dy - u_*dv_dxx - v_*dv_dxy) + 
                //         dphi_dx(1,i) * (dLy_dy/intPointWeightFunction(index) - du_dy*dv_dx - dv_dy*dv_dy - u_*dv_dxy - v_*dv_dyy)) * tARLQ_ / dens_;  
...
void Element<2>::getLagrangeMultipliersDifferentMesh(int ielem,double tPSPG2_,ublas::bounded_vector<double, 6> press, ublas::bounded_vector<double, 6> velx, ublas::bounded_vector<double, 6> vely){
    ...
    //Interpolates the velocity components and its spatial derivatives
            ...
            for (int i = 0; i < 6; i++){
                ...
                if (iTimeStep < 10){
                    LLx = -(dphi_dx(0,i) * dLx_dx/(1-intPointWeightFunction(index)) + dphi_dx(1,i) * dLx_dy/(1-intPointWeightFunction(index))) * tARLQ_ / dens_;
                    LLy = -(dphi_dx(0,i) * dLy_dx/(1-intPointWeightFunction(index)) + dphi_dx(1,i) * dLy_dy/(1-intPointWeightFunction(index))) * tARLQ_ / dens_;
                }else{
                    // LLx = -(dphi_dx(0,i) * (dLx_dx/(1-intPointWeightFunction(index)) + du_dx*du_dx + dv_dx*du_dy + u_*du_dxx + v_*du_dxy) + 
                    //         dphi_dx(1,i) * (dLx_dy/(1-intPointWeightFunction(index)) + du_dy*du_dx + dv_dy*du_dy + u_*dv_dxx + v_*dv_dxy)) * tARLQ_ / dens_;
                    // LLy = -(dphi_dx(0,i) * (dLy_dx/(1-intPointWeightFunction(index)) + du_dx*dv_dx + dv_dx*dv_dy + u_*du_dxy + v_*du_dyy) + 
                    //         dphi_dx(1,i) * (dLy_dy/(1-intPointWeightFunction(index)) + du_dy*dv_dx + dv_dy*dv_dy + u_*dv_dxy + v_*dv_dyy)) * tARLQ_ / dens_;
                ...
                arlequinStabVector(2*i  ) += (LLx) * weight_ * djac_ * (1-intPointWeightFunction(index)) ;
                arlequinStabVector(2*i+1) += (LLy) * weight_ * djac_ * (1-intPointWeightFunction(index)) ;
            };
        };
        index++;        
    };  
