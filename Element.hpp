...
public:
    ...
    void getLagrangeMultipliersDifferentMesh(int ielem,double tPSPG2_);
    ...
    for(typename NormalQuad::QuadratureListIt it = nQuad.begin(); 
        ...
        for (int i = 0; i < 6; i++){
            for (int j = 0; j < 6; j++){        
                ...
                //Fine
                LM = - ((u_ - umesh_) * dphi_dx(0,i) + (v_ - vmesh_) * dphi_dx(1,i))
                    * phi_(j) * tSUPG_;
            ...
            LMx = (((u_ - umesh_) * dphi_dx(0,i) + (v_ - vmesh_) * dphi_dx(1,i))
                   * lagMx_) * tSUPG_;
                
            LMy = (((u_ - umesh_) * dphi_dx(0,i) + (v_ - vmesh_) * dphi_dx(1,i))
                   * lagMy_) * tSUPG_;
...
//------------------------------------------------------------------------------
template<>
void Element<2>::getLagrangeMultipliersDifferentMesh(int ielem, double tPSPG2_){
        ...
        if ((intPointCorrespElem(index) == ielem)){
            ...
            for (int i = 0; i < 6; i++){
                for (int j = 0; j < 6; j++){        
                    ...
                    //Coarse
                    double LM = 0.;
                    LM = ((u_ - umesh_) * dphi_dx(0,j) + (v_ - vmesh_) * dphi_dx(0,j))
                        * phiLM_(i) * tSUPG_;
                ...
                LMx = ((u_ - umesh_) * dphi_dx(0,i) + 
                       (v_ - vmesh_) * dphi_dx(1,i)) * lagMx_ * tPSPG2_;
                
                LMy = ((u_ - umesh_) * dphi_dx(0,i) + 
                       (v_ - vmesh_) * dphi_dx(1,i)) * lagMy_ * tPSPG2_;
    ...
    // std::cout << "ELEM " << index_ << std::endl;
    // for (int i=0; i<12; i++){
    //     for (int j=0; j<12; j++){
    //         std::cout << lagrMultMatrix(i,j) << " " ;
    //     };
    //     std::cout << std::endl;
    // };
