...
NavierStokes::NavierStokes(int matid, int dim, REAL density, REAL viscosity) : Stokes(matid,dim,density,viscosity) {
...
void NavierStokes::ComputeStiffness(int &index, IntPointData &data, MatrixDouble &Stiffness){
    ...
    for (int i = nphi; i-- ; ){        
        ...
        for (int j = nphi; j-- ; ){           
            ...
            //Convection matrix
            REAL C = (wSUPGj * shapeFi + wSUPGi * wSUPGj * this->tSUPG_) * fDensity;
            ...
            for (int k = DIM; k--;  ){
                ...
                for (int l = DIM; l--; ){
                    ...
                    REAL Cuu = (shapeFij * data.fDSolDx(k,l) + 
                ...
                //PSPG stabilization
                REAL G = data.fDPhiX0(k,i) * wSUPGj * this->tPSPG_;
                REAL Guu = 0.;
...
void NavierStokes::ComputeResidual(int &index, IntPointData &data, VecDouble &Rhs){
    ...
    for (int i = nphi; i--; ){
        ...
        for (int k = DIM; k--; ){
            ...
            //Pressure + SUPG
            REAL P = - (data.fDPhiX0(k,i) * data.fSol[DIM]);
